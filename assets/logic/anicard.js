...
import { WorkShopData_Card } from './workshop_dataset.js';
...
function openWorkshopDataset(){
    $.ajax({
        url: "./assets/static/workshop_dataset.html",
        type: "get",
        contentType: "text/html",
        dataType: "html",
        success: function (res) {
            document.querySelector("main").innerHTML = res;
            workshopDataset_loading();
        }
    });
}
...
function Homepage_ex_loading() {
    ...
    navWorkshop.innerHTML=language_zh?language.WORKSHOP_ZH:language.WORKSHOP_EN;
...
const workshop_dataset_url = "./assets/json/workshop_dataset.json";
function workshopDataset_loading(){
    const workshop_deck_node = document.querySelector(".workshop-deck");
    const empty_workshop_deck_node = document.querySelector("#empty-workshop-deck-single");

    $.getJSON(workshop_dataset_url, json => {

        $.each(json, (i, workshop_item) => {
            let {
                id
                // id, wk_title, wk_source
            } = workshop_item;

            let wk_object = new WorkShopData_Card(workshop_item);

            // insert card object to the deck
            wk_object.appendTo(workshop_deck_node, empty_workshop_deck_node);
        });
    });
}