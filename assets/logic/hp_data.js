...
import arrSubtraction from "../tools/arraySub.js"
import { language} from '../constans/language.js';
import {objectsEN} from "../map/objectsEnMap.js"
...
class CardsFilter {
    ...
    filtering_method(obj = {}) {
        ...
        if (this._spec_tag.length > 0) {
            //keywords
            let originKey 
            originKey=language_zh?language.EL_ORIGIN_KEY_ZH:language.EL_ORIGIN_KEY_EN
            let newKey=arrSubtraction(originKey,this._keywords)
            let num=1;
            let correct = '';
            let all='';
            let singleItem='';
            if (obj.hasOwnProperty("eg_content")) {
                for(let i=0;i<newKey.length;i++){
                    all=obj["eg_content"][0].slice(0,obj["eg_content"][0].length-1)
                    if(all==="all"&&newKey.length>0){
                        num=0
                        break;
                    }
                    singleItem=language_zh?objectsEN[newKey[i]]:newKey[i]
                    correct = singleItem.toLowerCase().replace(" ", "-");
                    if (obj["eg_content"].indexOf(correct) > -1) {
                        num=0
                        break;
                    }
                }
